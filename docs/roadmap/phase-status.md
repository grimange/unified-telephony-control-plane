...
**RMA-D Archive Target and Secret-Reference Authority is COMPLETE /
AUTHENTICATED-API-PROVEN.** Its initial acceptance was FIXTURE-BLOCKED because no
canonical route could enroll a user into another tenant, making a tenant created
through `POST /api/v1/admin/tenants` unjoinable. That identity repair is
implemented, published, deployed, and now **fully resolved and live-proven**:
explicit `tenant_id` membership creation uses the dedicated platform-scoped
`platform.memberships.manage` capability (mapped only to `platform-admin`), while
the absent-field path retains active-tenant `tenant.memberships.manage` behavior.
The 2026-09-02 reproof exercised the whole canonical chain on the repaired digest:
platform-admin login, canonical tenant `B` creation, explicit platform enrollment
of the same user as `tenant-admin` in `B`, normal tenant-context selection of `B`
and back to `A`, a draft archive target, a `422` rejection of activation without a
credential, a write-only encrypted credential reference (`DECRYPTION_MATCH=PASS`,
`ENCRYPTED_NOT_CLEARTEXT=PASS`, fingerprint equal to SHA-256 of the submitted
secret, exactly one credential row), activation, owner `GET`/list, a real
cross-tenant `404` on the genuine tenant-A target from tenant `B` with the target
absent from `B`'s list, owner recovery to `200`, four correlated audit and outbox
records, zero cleartext-secret occurrences anywhere with `secret_fingerprint` and
`credential_reference_id` surviving PayloadSafety, and retirement with
`retired → active` rejected `422`. No storage provider participated and no RMA-E,
RMA-F, or RMA-G authority appeared. No RMA-D proof gap remains. See the
[RMA-D authenticated-API acceptance](../evidence/rma/rma-d-archive-target-and-secret-reference-authority-focused-deployed-authenticated-api-acceptance-passed.md),
the historical [cross-tenant fixture blocker](../evidence/rma/rma-d-archive-target-acceptance-cross-tenant-fixture-blocker.md),
and the [platform membership enrollment authority repair](../evidence/identity/platform-membership-enrollment-authority-repair.md).
...
44-byte zero-PCM content-independence boundary. No RMA-C proof gap remains. **RMA-D Archive Target and Secret-Reference Authority is COMPLETE / AUTHENTICATED-API-PROVEN**; the initial cross-tenant fixture blocker is closed by the deployed platform membership-enrollment authority and the full canonical chain is live-proven. See the [RMA-D authenticated-API acceptance](../evidence/rma/rma-d-archive-target-and-secret-reference-authority-focused-deployed-authenticated-api-acceptance-passed.md) and the historical [cross-tenant fixture blocker](../evidence/rma/rma-d-archive-target-acceptance-cross-tenant-fixture-blocker.md). RMA-E through RMA-H remain not started. RMA does not technically
gate A0, and

...
**T3-S2A repository implementation is Ready for Live Proof.** [`docs/evidence/t3/t3-s2a-asterisk-sip-application-dialog-authority.md`](../evidence/t3/t3-s2a-asterisk-sip-application-dialog-authority.md) records the bounded prerequisite for T3-S2 media mediation: one canonical internal Asterisk SIP destination (`asterisk-sip.utcp-runtime.svc.cluster.local:5060`), an explicit `autoload=no` PJSIP module set, one UDP `5060` internal transport, a Kamailio-facing `from-kamailio` endpoint with `direct_media=no`, a dedicated ClusterIP SIP Service separate from ARI, exact reciprocal Kamailio/Asterisk SIP NetworkPolicies, a local-overlay-only `9900` Answer/Echo/Hangup media fixture, and a Kamailio application-dialog route seam that authenticates initial INVITEs, applies Record-Route, uses loose routing for sequential requests, handles ACK/CANCEL/BYE/UPDATE explicitly, preserves unsupported-method `405`, and returns `503 Application Runtime Unavailable` when the canonical Asterisk target cannot be relayed. REGISTER remains unchanged, no rtpengine operation or SDP rewriting is present, no public SIP surface was added, and no Kubernetes apply or workload restart was performed. T3-S1 is Complete, T3-S2 media mediation is Not Started, and T3 remains In Progress. **The T3-S2A live signaling proof at `ab5ab55` is INCOMPLETE, blocked by `PRODUCT_DEFECT-5`, and is recorded in [`docs/evidence/t3/t3-s2a-asterisk-sip-application-dialog-live-proof.md`](../evidence/t3/t3-s2a-asterisk-sip-application-dialog-live-proof.md).** The Asterisk half is deployed and proven live: the rebuilt image (index digest `sha256:14dab605...`, `linux/amd64` `sha256:307acf45...`, running `imageID` matching exactly) loads every committed PJSIP module with `autoload=no` retained and ARI still available; exactly one PJSIP transport `transport-udp-internal` is active on UDP `0.0.0.0:5060` with no TCP, TLS, WS, or WSS SIP and no HostPort or node/host socket; the `anonymous` endpoint resolves `context=from-kamailio` with `direct_media=false` and `allow=(ulaw)`; the live `from-kamailio` dialplan carries `9900` (`Answer`/`Echo`/`Hangup`) sourced only from `extensions.local.conf` plus a deterministic `_.` to `Hangup(21)` from the image, with `9900` absent from the image itself; `Service/utcp-runtime/asterisk-sip` is ClusterIP UDP `5060` with exactly one Ready endpoint equal to the Asterisk Pod IP and `asterisk-ari-b` correctly excluded; Kamailio resolves the canonical Service DNS through cluster DNS; and the reciprocal NetworkPolicy corridor allows the real Kamailio identity (`SIP/2.0 200 OK` in `0.014s`) while denying an unauthorized `default`-namespace source with a bounded timeout, with default-deny intact and the ARI policy still separate on TCP `8088`. Only the six T3-S2A resources were applied; the pre-existing unrelated `utcp-application-config` drift (`APP_URL`, `BROADCAST_CONNECTION`, `REVERB_*`) was explicitly excluded. REGISTER is fully preserved (canonical `401` digest challenge live, running Kamailio configuration byte-identical at `sha256 6e85abaf...`), no rtpengine mediation occurred (sessions and port counters unchanged, zero offer/answer/delete operations), no public SIP surface exists, and no canonical state changed. **`PRODUCT_DEFECT-5`:** `route[APPLICATION_DIALOG]` in `infrastructure/kubernetes/base/platform/kamailio-configmap.yaml` invokes `has_totag()` without `loadmodule "siputils.so"`, so Kamailio fails to parse its own configuration (`failed to find command has_totag`, parse error at line 114) and every new Pod exits `255` into `CrashLoopBackOff`. Validated with Kamailio's own parser: adding that single `loadmodule` line makes the configuration parse cleanly, and `siputils.so` is already present in the image. Both `make kamailio-signaling-config-check` and `make kamailio-signaling-config-check-test` pass despite this, so the bounded correction is the one-line `loadmodule` plus a static assertion that parses the rendered configuration with Kamailio itself. Consequently the entire SIP dialog corridor — authentication challenge, authenticated INVITE, SDP-bearing answer, Record-Route, ACK, BYE, CANCEL, and the Asterisk-unavailable `503` contract — remains unproven and is not claimed. Kamailio currently stays available only because the pre-existing Pod is still running under rolling-update protection; **any eviction or reschedule of that Pod will stop SIP signaling until `PRODUCT_DEFECT-5` is corrected.** T3-S2 media mediation is Not Started and T3 remains In Progress.

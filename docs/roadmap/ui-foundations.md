...
- Shared table abstraction, broader validation, broader mutation adoption, and catalog-driven adapter-form systems do not yet exist.
...
- Controlled browser proof for the UI-C1 request state, notifications, and on-demand RuntimeNode detail loading slice is complete (UI-C2 evidence above). UI-C4 proved the UI-C3 URL-backed list state, history restoration, normalization, pagination reset, and RuntimeNode request-preservation behavior, but surfaced a blocking stale-response defect (below) that must be corrected before UI-C list state is considered acceptance-clean.
- Correct the Users list stale-response overwrite: guard the rendered `users`/`userPagination` module state against superseded responses (for example by applying the results only inside the `useAsyncResource` `requestId`-guarded path, or writing them through the resource `state.data` the view then renders), then re-run the focused Users stale-response proof.

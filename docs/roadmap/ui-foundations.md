...
- UI-D21 repository correction (evidence: [`docs/evidence/ui/ui-d21-reconciliation-noop-event-suppression.md`](../evidence/ui/ui-d21-reconciliation-noop-event-suppression.md)) fixes the UI-D20 producer storm at `ReconciliationRepository::claimDue()` and `markResult()`: routine lease claims no longer emit `runtime_reconciliation.reconciliation_started`, result handling compares the prior and resulting canonical transition fingerprint (`status`, `desired_generation`, `observed_generation`, `last_operation_id`, `blocked_reason`) inside the authoritative transaction, unchanged fingerprints append zero outbox rows, and real convergence, drift, new linked operation, failure or blocked-reason change, and retry-state transitions still append exactly one canonical `runtime_reconciliation.*` aggregate event. Producer-backed tests now prove no-op claim/result cycles, claim-only lease changes, repeated equal waiting/failure/retry/converged/operation-required results, and 20 stable rows over 5 no-op polling cycles produce zero new reconciliation events, while PostgreSQL coverage proves the invariant, fencing, rollback, and one-event real transition behavior through the established control-plane target. The browser channel and metadata-only envelope are unchanged, no frontend debounce or downstream suppression was introduced, automatic reconciliation remains enabled, and historical outbox rows are preserved for canonical dispatcher drain. UI-D remains In Progress pending a corrected natural Runtime Reconciliation live proof after deployment and backlog drain, plus the separate Audit API.
- UI-D22 controlled natural live proof (evidence: [`docs/evidence/ui/ui-d22-reconciliation-quiescence-live-proof.md`](../evidence/ui/ui-d22-reconciliation-quiescence-live-proof.md), api `sha256:a3c8c6ce…` built from `5e31943` with `org.opencontainers.image.revision=5e31943`; web unchanged at `sha256:5947ec57…` / `revision=d9fe346`) **closes the UI-D20 event-storm blocker and completes reconciliation live acceptance**. All 12 API-image Deployments plus web were enumerated from the rendered manifests and explicitly restarted (with `crictl rmi` on all three k3d nodes for the `IfNotPresent` fence worker), giving zero skew — 13 API pods on the `5e31943` digest, 1 web pod on the proven `d9fe346` bundle — and migration completed on `BROADCAST_CONNECTION=log` without Reverb credentials. **No-op production stopped**: with scheduler and reconciler at normal replicas, `rr_total` froze at 28101 immediately after rollout while the preserved UI-D20 backlog drained canonically 12240 → **0** over ~57 min (115 samples, ~3.4/s dispatch, 0 failed, no row deleted or manually marked); +60 reconciler claims in 20 s and 200/200 `waiting` results produced 0 outbox rows. **Primary browser quiescence**: 150 s / ~50 poll cycles / +350 claims on the mounted route with a selection held → 0 events, 0 list rereads, 0 detail rereads (UI-D20 saw 110 idle rereads in 30 s); route entry issued exactly 1 list request (was 11). **Each real fingerprint transition still emits exactly once**: an unrelated node Activate produced 3 fingerprint changes → +3 outbox rows → 3 notifications → 3 list rereads → 0 selected-detail rereads; the selected node produced 5 changes (drift → 3 distinct operation_required, each with a distinct real `runtime_operation_id`, → converged) → +5 rows → 5 list + 5 selected-detail rereads → 0 unrelated, with status/generations/operation/failure all read from the detail HTTP response and the envelope carrying only the eight approved metadata keys with no status/generation/drift/state/failure/credential data; 110 s of subsequent polling with the row selected emitted 0 repeat events. Regressions held: reconnect performed one bounded resync (1 list + 1 detail + 0 unrelated, page/filters/selection preserved); route leave sent `pusher:unsubscribe`, kept the shared socket, issued 0 reconciliation requests, then 1 fresh list + 1 subscription on return; tenant switch left the old channel, cleared selection, loaded once with 0 detail requests, 0 new sockets and 0 leaked rows; a second context's real Local Tenant transition (server-confirmed 3 outbox rows on the `local` channel) yielded 0 accepted events / 0 rereads / 0 leaked rows in the primary on Proof Tenant; logout closed the socket with 0 post-logout auth or reconciliation requests; storage stayed bounded to `utcp.appearance` + `pusherTransportTLS` with a zero-hit forbidden-substring scan; and 375px overflow was 0 in Light and Dark. All three transitioned disposable RuntimeNodes were restored to disabled, Reverb (cycled only for the reconnect regression) restored to 1, and pending outbox confirmed stable at 0 across a final idle window. UI-D remains In Progress for the canonical Audit read authority.
- UI-D23 repository implementation (evidence: [`docs/evidence/ui/ui-d23-audit-read-api.md`](../evidence/ui/ui-d23-audit-read-api.md)) adds tenant-scoped read-only Audit list and detail APIs backed by the existing append-only `control_plane_audit_records` table and `AuditRepository` append authority. The route reuses the existing `tenant.memberships.manage` tenant-admin capability because no narrower Audit-view capability exists, applies active-tenant database scoping, preserves the PostgreSQL append-only mutation trigger, and adds SQL-backed pagination, deterministic `occurred_at desc, id desc` ordering, validated stored-column filters, explicit list/detail resources, bounded immutable actor and subject references, safe outcome derivation, and metadata redaction without exposing raw request bodies, credentials, tokens, cookies, authorization headers, stack traces, outbox bodies, desired state, or observed state. Feature tests cover authorization, tenant isolation, filter validation, exact safe contracts, GET immutability, absence of write routes, and non-scaling list query counts; PostgreSQL endpoint coverage runs through the established control-plane target and proves the append-only trigger. UI-D remains In Progress pending the bounded Audit operational view and a repository-evidence decision on whether Audit requires realtime notifications.
- UI-D24 repository implementation (evidence: [`docs/evidence/ui/ui-d24-audit-view-implementation.md`](../evidence/ui/ui-d24-audit-view-implementation.md)) adds the read-only `/admin/audit-records` operational route and navigation using only `GET /api/v1/admin/audit-records` and `GET /api/v1/admin/audit-records/{id}`. The route requires an active tenant plus `tenant.memberships.manage`, uses server-backed pagination and the canonical Audit filters, preserves backend ordering, loads selected detail on demand with zero unrelated detail fan-out, implements explicit user refresh without polling or timers, renders only bounded actor, subject, outcome, reason, and safe metadata fields, excludes sensitive payloads, avoids Audit storage, and adds focused frontend tests plus responsive hygiene coverage. Audit realtime notifications are deliberately not implemented because no canonical Audit event/channel or live-update requirement has been established. UI-D remains In Progress pending focused natural Playwright MCP proof and final UI-D completion assessment.

...
- Controlled browser proof for the UI-C1 request state, notifications, and on-demand RuntimeNode detail loading slice is complete (UI-C2 evidence above). UI-C4 proved the UI-C3 URL-backed list state, history restoration, normalization, pagination reset, and RuntimeNode request-preservation behavior, but surfaced a blocking stale-response defect. UI-C5 corrected that defect in repository code with rendered Users rows and pagination bound to the guarded current-request result.
- Re-run only the focused Users stale-response browser proof to verify that delayed query A can no longer overwrite newer query B rows or pagination, then close the UI-C3 live-proof gap if it passes.

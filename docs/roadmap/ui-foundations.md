...
- Shared table abstraction, broader validation, broader mutation adoption, and frontend catalog-driven adapter-form rendering do not yet exist.
...
- Replace the bounded Asterisk-specific adapter-form branch by consuming the canonical server-published adapter configuration descriptors.
- Preserve one-time and write-only secret handling.
- Controlled browser proof for the UI-C1 request state, notifications, and on-demand RuntimeNode detail loading slice is complete (UI-C2 evidence above). UI-C4 proved the UI-C3 URL-backed list state, history restoration, normalization, pagination reset, and RuntimeNode request-preservation behavior, but surfaced a blocking stale-response defect. UI-C5 corrected that defect in repository code, and UI-C6 live-proved the correction — the UI-C3 stale-response gap is now closed (delayed obsolete query A no longer overwrites newer query B rows or pagination).
- Remaining UI-C scope: frontend consumption of RuntimeNode adapter descriptors with removal of the handwritten Asterisk branch, broader shared table/validation/mutation-workflow adoption, and any still-unstandardized management workflow.

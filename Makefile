...
phase-status-consistency-check: ## Check evidence impact markers and ledger reconciliation.
	@python3 scripts/repository/phase-status-consistency-check

phase-status-consistency-check-test: ## Run phase-status consistency mutation tests.
	@python3 scripts/repository/phase-status-consistency-check-test

# Preserve the existing explicit declaration omitted from the expanded list above.
.PHONY: control-plane-proof

...
T1 is complete (`PHASE_T1_COMPLETE`, evidence `docs/evidence/t1/kamailio-sip-over-wss-signaling.md`), and so is T2 Asterisk conference execution. The UI Foundation track UI-A through UI-E is complete (see `docs/roadmap/ui-foundations.md`). T5 convergence, failover, and recovery is in progress with only its final phase-closure evidence outstanding; T3 rtpengine browser media, V0, T4 FreeSWITCH parity, and R0 remain planned. `UTCP_PHASE` is `T1`, held deliberately as the CI-guarded marker for the authoritative completed phase rather than advanced automatically — `make repository-hygiene` fails unless it reads `T1` and `docs/roadmap/phase-status.md` marks T1 complete. `docs/roadmap/phase-status.md` is the authoritative status ledger. The paragraph below records the T1 corridor as originally delivered.

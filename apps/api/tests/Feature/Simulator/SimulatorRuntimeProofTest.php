...
final class SimulatorRuntimeProofTest extends TestCase
{
    ...
    public function test_steady_ready_lifecycle_converges_through_c3_workers_and_projection(): void
    {
        ...
        $this->assertSame(3, DB::table('runtime_event_receipts')->where('runtime_node_id', $nodeId)->where('status', 'processed')->count());
    ...
    public function test_retry_terminal_timeout_and_duplicate_scenarios_use_real_operation_and_event_paths(): void
    {
        ...
        $this->command();
        $this->publishAndNormalize();
        $this->assertSame(2, DB::table('simulator_scheduled_events')->where('runtime_node_id', $duplicateNode)->where('status', 'published')->count());
        $this->assertSame(1, DB::table('runtime_event_receipts')->where('runtime_node_id', $duplicateNode)->count());
        $this->assertSame(1, DB::table('runtime_observations')->where('runtime_node_id', $duplicateNode)->count());
     ...
     */
    private function createSimulatorNode(string $scenario, int $configurationGeneration = 1, array $parameters = []): array
    {
        ...
        foreach (['event.stream', 'runtime.configuration', 'runtime.observation'] as $capability) {

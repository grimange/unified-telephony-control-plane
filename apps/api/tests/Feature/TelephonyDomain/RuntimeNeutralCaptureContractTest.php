...
final class RuntimeNeutralCaptureContractTest extends TestCase
{
    public function test_capture_operations_use_only_neutral_identity_fields(): void
    {
        ...
        $captureRef = CaptureReference::forRecordingSession($sessionId)->canonical();
        $start = ['call_id' => 'call-1', 'leg_id' => 'leg-1', 'recording_session_id' => $sessionId, 'capture_ref' => $captureRef];
        $stop = ['call_id' => 'call-1', 'leg_id' => 'leg-1', 'recording_session_id' => $sessionId, 'capture_ref' => $captureRef];
        ...
        $this->assertSame($sessionId, substr($captureRef, strlen('utcp:capture/')));
        $this->assertNotSame('utcp:capture/'.md5($sessionId), $captureRef);
    ...
    {
        $captureRef = CaptureReference::forRecordingSession('f378c9e4c806daedb175c5d9f6173f20')->canonical();
